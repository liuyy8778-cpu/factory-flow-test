...
看到 `Ready` 後，用瀏覽器開 <http://localhost:3210>（用 `啟動.bat` 會自動開；直接 `npm run start` 則是 3000）。
...
| `PORT` | 連接埠 | `啟動.bat` 用 3210，避免跟其他程式撞號 |
...
把這套裝在一台常開的電腦或迷你主機，其他電腦用瀏覽器開 `http://主機IP:3210` 即可。
