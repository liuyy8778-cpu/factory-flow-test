...
## 附錄 D：簡易流程（2026-09-08，第一步完成）

辦公室兩人使用，日常只有四步：**進貨 → 派工（即開工）→ 出貨（即完工）→ 月結**。

- 側欄日常六項：今日看板、進貨、派工、出貨、月結 / 請款、往來廠商。其餘（來料規格庫、加工圖面庫、其他訂單、鐵桶登記簿、備份與復原）收進「進階」，功能不刪。
- 今日看板六格：待確認圖面、待派工、加工中、逾期未出貨、完工待出貨、出貨待請款。只讀，不改任何規則。
- 派工即開工：`dispatch_allocation` 多一個 `start:true`，工單直接以 `running` 建立並記 `started_at`；機台可留空，師傅必填。
- 出貨即完工：`/api/production` 新增 `complete_ship`。一個交易內：寫一筆「出貨即完工」的產量回報、把工單設為 `completed`（良品 = 已出貨 + 本次出貨；不良 = 派工數 − 良品）、建立出貨單、更新已出貨數、寫鐵桶帳。守門：工單未完工、出貨數 ≤ 派工數 − 已出貨。
- **規則變更（唯一一條）**：原本「完工時良品必須等於派工數」在簡易流程不適用；差額記為不良。原本的開工／回報／確認完工按鈕仍在，走舊流程的工單不受影響。
- 排隊中：② 四聯單列印、③ 月結代工費用報表、④ 進貨標籤。等使用者提供樣本。

---

## 附錄 E：系列圖面（2026-09-08）

客戶的圖幾乎都是「一張表管一整個系列」（例如 1/2" 手動套筒 6～32 mm）。研磨在別家，車床只看七格。

- 資料表：`series_drawings`（表頭：客戶、圖號、版本、圖名、材質、分、日期、圖檔）、`series_drawing_rows`（一列一個公稱尺寸：型式、車哪一頭、車削尺寸、階段距、總長、雙溝槽位置、螺孔深、備註）。migration 0012，含稽核與禁刪 trigger。
- 存檔時每列依「分 + 公稱尺寸」對應到來料規格（M10 與 10 視為相同；6P 與 12P 共用同一筆圖面），自動產生原格式 `drawings`（ends-v1）並設為慣用：
  - 車螺孔外徑（A 型）：工作端 machine，加工長度 = 總長 − 階段距；方孔端不加工。
  - 車方孔外徑（B 型）：方孔端 machine，加工長度 = 階段距；工作端不加工。
  - 雙溝槽位置有值 → 溝槽 custom「雙圓溝，間距 H」；空白 → 無溝。
  - 原守門照跑：總長 ≤ 來料長、車削尺寸 ≤ 來料外徑、加工長度 ≤ 總長。任何一列失敗整張不存。
  - `data.series` 帶七格原文，派工標籤改印七格；沒有 series 的舊圖面維持原版面。
- 沒有來料規格的尺寸先存起來，規格建好後按「補對應」（`regenerate`）。
- 不提供修改，改尺寸用新版本重建；「停用整個系列」會停用它產生的所有圖面。
- 輸入：可從 Excel 貼上（Tab 或空白分隔，欄位順序固定），也可逐列打。
- 側欄日常項目多一個「圖面」，共七項。
